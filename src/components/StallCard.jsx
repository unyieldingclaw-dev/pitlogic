const STALL_BARS = [
  ...
  { label: 'Poultry & fish',time: 'minimal',       width: 8,  color: '#aaa'         },
];
...
export default function StallCard() {
  return (
    <div className="card" style={{ marginTop: '1.5rem', borderColor: 'rgba(186,117,23,.2)', background: 'rgba(186,117,23,.03)' }}>
      <div style={{ fontWeight: 500, fontSize: 14, marginBottom: '.75rem' }}>Understanding the stall</div>
      ...
      {STALL_BARS.map((b, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, fontSize: 13 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--text2)', minWidth: 55, textAlign: 'right' }}>{b.time}</span>
        </div>
      ))}

      <hr className="divider" />
      <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)', marginBottom: 8 }}>Three ways to handle it</div>
      ...
      <hr className="divider" />
      <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)', marginBottom: 8 }}>What affects stall length</div>
